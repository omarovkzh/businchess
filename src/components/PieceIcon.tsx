// Cburnett chess piece set — classic Staunton-style SVGs used by Lichess.
// Public domain (GFDL/BSD). Adapted to render as currentColor where possible
// so the stroke can be themed. Each piece is rendered into a 45×45 viewBox.

import type { Color, PieceSymbol } from "chess.js";

type PieceSVG = (props: { className?: string }) => JSX.Element;
...
const WhiteKing: PieceSVG = ({ className }) =>
  wrap(
    <g
      fill="none"
      fillRule="evenodd"
...
const WhiteQueen: PieceSVG = ({ className }) =>
  wrap(
...
const WhiteRook: PieceSVG = ({ className }) =>
  wrap(
...
const WhiteBishop: PieceSVG = ({ className }) =>
  wrap(
...
const WhiteKnight: PieceSVG = ({ className }) =>
  wrap(
    <g fill="none" fillRule="evenodd" stroke="#000" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
...
const WhitePawn: PieceSVG = ({ className }) =>
  wrap(
    <path
...
const BlackKing: PieceSVG = ({ className }) =>
  wrap(
    <g fill="none" fillRule="evenodd" stroke="#000" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
...
const BlackQueen: PieceSVG = ({ className }) =>
  wrap(
...
const BlackRook: PieceSVG = ({ className }) =>
  wrap(
...
const BlackBishop: PieceSVG = ({ className }) =>
  wrap(
    <g fill="none" fillRule="evenodd" stroke="#000" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
...
const BlackKnight: PieceSVG = ({ className }) =>
  wrap(
    <g fill="none" fillRule="evenodd" stroke="#000" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M24.55 10.4l-.45 1.45.5.15c3.15 1 5.65 2.49 7.9 6.75 2.25 4.25 3.05 10.31 2.25 20.25l-.05.5h2.25l.05-.5c.8-10.06-.04-16.32-2.4-20.74-2.36-4.42-5.16-6.04-8.4-7.04l-.5-.15z" fill="#fff" stroke="none" />
    </g>,
    className,
  );

const BlackPawn: PieceSVG = ({ className }) =>
  wrap(
    <path
      d="M22.5 9c-2.21 0-4 1.79-4 4 0 .89.29 1.71.78 2.38C17.33 16.5 16 18.59 16 21c0 2.03.94 3.84 2.41 5.03-3 1.06-7.41 5.55-7.41 13.47h23c0-7.92-4.41-12.41-7.41-13.47C28.06 24.84 29 23.03 29 21c0-2.41-1.33-4.5-3.28-5.62.49-.67.78-1.49.78-2.38 0-2.21-1.79-4-4-4z"
      fill="#000"
...
const PIECE_SVGS: Record<string, PieceSVG> = {
  wK: WhiteKing,   wQ: WhiteQueen,   wR: WhiteRook,   wB: WhiteBishop,   wN: WhiteKnight,   wP: WhitePawn,
  bK: BlackKing,   bQ: BlackQueen,   bR: BlackRook,   bB: BlackBishop,   bN: BlackKnight,   bP: BlackPawn,
};
